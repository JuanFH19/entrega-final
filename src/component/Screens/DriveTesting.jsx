...
const DriveTesting = () => {
  ...
  const handleSubmit = () => {
    if (nombre === '' || telefono === '' || correo === '') {
      ...
      try {
        firebase.db.collection('solicitudes').add({
          nombre,
          telefono,
          correo,
        });
        Alert.alert(
          'Solicitud enviada',
          'Gracias por solicitar una prueba de manejo, en breve nos pondremos en contacto contigo',
        );
        setNombre('');
        setTelefono('');
        setCorreo('');
  ...
  return (
    <View style={styles.container}>
      ...
      <Button style={styles.button} onPress={() => handleSubmit()}>
        <Text style={styles.buttonText}>Solicitar prueba</Text>
      </Button>
    </View>
  );
};
